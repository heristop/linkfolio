import { renderContentPlainText } from "../lib/projectContent";
import { appUrl, isShowcase } from "../lib/siteMeta";

export const dynamic = "force-static";

/**
 * The full corpus for answer engines, as the llmstxt convention intends:
 * llms.txt indexes, llms-full.txt carries the text. The comparison and FAQ
 * come from projectContent.ts so this document cannot disagree with the
 * README or the /docs page about what the project is.
 *
 * Same showcase-only gating as llms.txt: this document describes the
 * project, not a fork's personal page, so a personal deployment 404s.
 */
const body = `# Linkfolio

> A self-hosted, open-source link-in-bio page built with Next.js and Tailwind CSS.

Linkfolio is a customizable link-in-bio page that deploys as a standalone template or installs as a package into an existing Next.js project. You host it yourself: there is no account, no subscription, and no third party between you and your visitors.
...
- Package: https://www.npmjs.com/package/linkfolio
- Landing page: ${appUrl}
- Demo: ${appUrl}/demo
